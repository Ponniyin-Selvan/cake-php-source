...
"10" => "Harare")
	...
	function lookupIp($ip) {
        vendor("geoipcity");
		include_once(APP.'vendors'.DS.'geoipregionvars.php');

        $gi = geoip_open("files/common/geolitecity.dat", GEOIP_STANDARD);
        $result = geoip_record_by_addr($gi, $ip);
        geoip_close($gi);
		$result_array = get_object_vars($result);
        if ($result_array && count($result_array) > 0) {
	       $result_array['country_name'] = (array_key_exists('country_name', $result_array) ? $result_array['country_name'] : null);
   		   $result_array['region_name'] = (array_key_exists('region_name', $result_array) ? $result_array['region_name'] : null);
   		   $result_array['city'] = (array_key_exists('city', $result_array) ? $result_array['city'] : null);

		   if (array_key_exists('country_code', $result_array)
		       && array_key_exists($result_array['country_code'], $this->GEOIP_REGION_NAME)) {
		       	$country_array = $this->GEOIP_REGION_NAME[$result_array['country_code']];
			    if (array_key_exists('region', $result_array) &&
			       array_key_exists($result_array['region'], $country_array)) {
				   $result_array['region_name'] = $country_array[$result_array['region']];
			    }
           }
           $result_array['offset'] = $result_array['longitude'] / 15;
        } else {
	       $result_array['country_name'] =  null;
   		   $result_array['region_name'] =  null;
   		   $result_array['city'] = null;
   		   $result_array['longitude'] = null;
   		   $result_array['latitude'] = null;
        }
        return $result_array;
    }
}
