 ...
 */
	 function display() {
		  ...
		  if (!empty($path[0])) {
			  $page = $path[0];
			  if ($path[0] == "others") {
		  		$this->layout = 'others'; // Layout without sidebars
			  }
		  }
		  ...
		  $title = $this->to_seo_title($page);
		  if ($subpage != null && $subpage != "") {
		  	  $title = $title." - ".$this->to_seo_title($subpage);
		  }
	 ...
	 function to_seo_title($title) {
		  $seo_title = str_replace("-", " ", $title);		  
		  $seo_title = str_replace(".html", "", $seo_title);		  
