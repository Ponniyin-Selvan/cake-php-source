 ...
 */
	function _getLayoutFileName() {
		...
		// added for .thtml viewPath fallback
		foreach($paths->viewPaths as $path) {
			if (file_exists($path . 'layouts' . DS  . $type . $this->layout . '.thtml')) {
				$layoutFileName = $path . 'layouts' . DS . $type . $this->layout . '.thtml';
				return $layoutFileName;
			}
		}
...
/**
 * Returns filename of given action's template file (.tpl) as a string. CamelCased action names will be under_scored! This means that you can have LongActionNames that refer to long_action_names.thtml views.
 *  - added: will also return .thtml templates in viewPath paths for fallback
 *
 ...
 */
	function _getViewFileName($action) {
		...
		// added for .thtml viewPath fallback
		foreach($paths->viewPaths as $path) {
			if (file_exists($path . $this->viewPath . DS . $type . $action . '.thtml')) {
				$viewFileName = $path . $this->viewPath . DS . $type . $action . '.thtml';
				return $viewFileName;
			}
		}
